...
public class VmSchedulerSpaceShared extends VmScheduler {
	...
	/*public boolean allocatePesForVm(Vm vm, List<Double> mipsShare) {
		// if there is no enough free PEs, fails
	        ...
	        System.out.print("error"+getFreePes().size());
                return false;
		}
		...
		setAvailableMips(getAvailableMips() - totalMips);
                ...
                vm.Davailablemips=getAvailableMips();
                vm.DusedMips=getFreePes().size();
		return true;
	}
        ...
        */
		...
		if (mipsShare.size() > selectedPes.size()) {
			return false;
                ...
                for(int i=0;i<selectedPes.size();i++)
                {  
                    if(selectedPes.get(i).getMips()==0.0)
                {getFreePes().remove(selectedPes.get(i));
                }
                }
